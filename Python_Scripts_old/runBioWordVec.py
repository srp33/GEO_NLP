...
print(f"REDUCE SET VALUE == {reduced_set}")
...
print_timestamp("Loading BioWordVec")
...
for query in range(6,7):
    for keywordExtractor in ["TopicRank", "TfIdf", "KPMiner", "YAKE", "TextRank", "SingleRank", "TopicalPageRank", "PositionRank", "MultipartiteRank"]:
        mp = multiprocessing.Process(target=findSimilarity, args=(keywordExtractor, "BioWordVec", model, candidate_articles, query, num_keywords, vector_size))
        mp.start()
        #mp.join()
