...
print_timestamp("Training FastText SkipGram model")
model = trainFastTextModel(vector_size, 'skipgram')

print_timestamp("FastTextSkipGram model trained")
...
for query in range(6,7):
    for keywordExtractor in ["TopicRank", "TfIdf", "KPMiner", "YAKE", "TextRank", "SingleRank", "TopicalPageRank", "PositionRank", "MultipartiteRank"]:
        mp = multiprocessing.Process(target=findSimilarity, args=(keywordExtractor, "FTSkipGram", model, candidate_articles, query, num_keywords, vector_size))
        mp.start()
        #mp.join()
