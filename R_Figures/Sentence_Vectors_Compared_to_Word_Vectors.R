...
#word embeddings v sentence vectors
vector_method = filter(data1, Model_Type == 'fasttext__cbow' | Model_Type == 'fasttext__skipgram')

v <- ggplot(vector_method, aes(x = Vector_Method, y= Accuracy)) + 
  geom_col()+
  #facet_wrap(~Query)+
  theme_bw()
print(v+facet_wrap(~Query))
